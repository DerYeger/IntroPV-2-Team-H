...
public class BuddyKGV {
    ...
    public static void main(String[] args) {
        ...
        final GlobalRef<ConcurrentHashMap<Integer, Buddy>> buddyMap = new GlobalRef<>(places(), () -> new ConcurrentHashMap<>(m, 1, workers));
        ...
        finish(() -> {
            for (final Place place : places()) {
                asyncAt(place, () -> {
                    ...
                    final ConcurrentHashMap<Integer, Buddy> placeBuddyMap = buddyMap.get();
                        ...
                        async(() -> {
                            ...
                            for (int y = start; y < end; y++) {
                                for (int x = 0; x < n; x++) {
                                    ...
                                    if (minKgv == -1) {
                                        placeResults[y][x] = placeBuddyMap.computeIfAbsent(number, buddy -> findMaxBuddy(number, b, placeKGVBuffer));
                                    } else {
                                        placeResults[y][x] = placeBuddyMap.computeIfAbsent(number, buddy -> findBuddy(number, b, minKgv, placeKGVBuffer));
                                    }
                                }
                            }
                        });
    ...
    private static Buddy findMaxBuddy(final int number,
        ...
        for (int y = 0; y < b.length; y++) {
            for (int x = 0; x < b.length; x++) {
                ...
                if (number * other < maxKGV) continue;
    ...
    private static Buddy findBuddy(final int number,
        ...
        for (int y = 0; y < b.length; y++) {
            for (int x = 0; x < b.length; x++) {
                ...
                if (number * other < minKgv) continue;
